...
export async function GET() {
  ...
  for (const projectId of PROJECT_IDS) {
    try {
      const { data: project } = await client.from("projects").select("id,nama_desa,kecamatan").eq("id", projectId).single();
  ...
  return NextResponse.json({ queued: results.length, results }, { headers: { "Cache-Control": "no-store" } });
}
